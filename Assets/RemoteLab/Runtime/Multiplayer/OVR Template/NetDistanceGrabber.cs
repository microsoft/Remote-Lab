...
public class NetDistanceGrabber : OVRGrabber
{
    ...
    NetDistanceGrabber m_otherHand;

    protected NetDistanceGrabbable m_target;
    ...
    protected override void Start()
    {
        ...
        NetDistanceGrabber[] grabbers = FindObjectsOfType<NetDistanceGrabber>();
    ...
    {
        NetDistanceGrabbable closestGrabbable = m_target;
        ...
        if (closestGrabbable != null)
        {
            ...
            {
                ((NetDistanceGrabber)closestGrabbable.grabbedBy).OffhandGrabbed(closestGrabbable);
            }
    ...
    static private NetDistanceGrabbable HitInfoToGrabbable(RaycastHit hitInfo)
    {
        if (hitInfo.collider != null)
        {
            GameObject go = hitInfo.collider.gameObject;
            return go.GetComponent<NetDistanceGrabbable>() ?? go.GetComponentInParent<NetDistanceGrabbable>();
        }
        return null;
    }

    protected bool FindTarget(out NetDistanceGrabbable dgOut, out Collider collOut)
    {
        dgOut = null;
        collOut = null;
        ...
        {
            NetDistanceGrabbable grabbable = cg as NetDistanceGrabbable;
    ...
    protected bool FindTargetWithSpherecast(out NetDistanceGrabbable dgOut, out Collider collOut)
    {
        dgOut = null;
        collOut = null;
        ...
        {
            NetDistanceGrabbable grabbable = null;
            Collider hitCollider = null;
            if (hitInfo.collider != null)
            {
                grabbable = hitInfo.collider.gameObject.GetComponentInParent<NetDistanceGrabbable>();
            ...
            if (grabbable != null && m_preventGrabThroughWalls)
            {
                ...
                if (Physics.Raycast(ray, out obstructionHitInfo, m_maxGrabDistance, 1 << m_obstructionLayer, QueryTriggerInteraction.Ignore))
                {
                    NetDistanceGrabbable obstruction = null;
                    if (hitInfo.collider != null)
                    {
                        obstruction = obstructionHitInfo.collider.gameObject.GetComponentInParent<NetDistanceGrabbable>();
                    }
